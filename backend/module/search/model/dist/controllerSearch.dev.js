...
function searchMenu() {
  ...
  console.log(chairSearch); //   chairSearch= cleanSQLInyection(chairSearch);
...
function cargarSearch() {
  ajaxPromise(friendlyModFunc("search", "autoComplete"), "GET", "JSON").then(function (datas) {
    // alert(datas);
    $("#inputSearch").empty(); // console.log(datas);
    ...
    var section = $("<input>").attr({
      id: "inputSearch"
    }).appendTo("#formuID").easyAutocomplete(options); // $("#inputSearch").easyAutocomplete(options);
