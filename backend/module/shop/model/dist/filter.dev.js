...
function queryPrice(minPrice, maxPrice) {
  ...
  return ""; // return " country BETWEEN'" + minPrice + "' AND '" + maxPrice + "' AND";
}
...
function chairsFormats() {
  ...
  for (var i = 0; i < formatsMatrix.length; i++) {
    if (sessionStorage.getItem(formatsMatrix[i]) == formatsMatrix[i]) {
      ...
      chairformats += " formats  LIKE '%" + sessionStorage.getItem(formatsMatrix[i]) + "%' OR";
    }
  }

  console.log(chairformats);
  chairformats = chairformats.split(" ") // separa el string según espacios en blanco
...
function searchFilter() {
  ...
  var chairQUERY = chairNameMovie + chairDirector + chairYear + chairFormats + chairGenere + chairCountry + chairPrice;
  console.log(chairQUERY);
  chairQUERY = chairQUERY.split(" ") // separa el string según espacios en blanco
  .slice(0, -1) // toma todos los elementos menos el último
  .join(" "); // vuelve a armar el string

  console.log(chairQUERY); // return false;
