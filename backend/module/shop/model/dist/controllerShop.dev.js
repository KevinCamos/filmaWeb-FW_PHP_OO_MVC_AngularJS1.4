...
importarScript(GENERAL_PATH + "module/shop/model/orderby.js"); /////FUNCIONES PARA CARGAR LOCALSTORAGE DE LOS FORMATOS

function clickerItems(itemString, id) {
  if (sessionStorage.getItem(itemString) == null || sessionStorage.getItem(itemString) == "null") {
...
function loadDivsProducts() {
  var section = $("<section></section>").attr({
    id: "port-sec"
  }).appendTo("#listShop");
...
function divsProduct(urls, id) {
  ...
  idUser == null ? idUser = -1 : idUser = idUser; // alert(idUser);
  // alert(id);
  ...
  ajaxPromise(friendlyModFunc("shop", "openProduct"), "GET", "JSON", {
    ...
    category = category[0]; //// .hide PER A OCULTAR EL FORMULARI QUE REOBRIREM AL EIXIR
    ...
    for (var clave in category) {
      ...
      if (clave == "img") {
        ...
        }).addClass("productShop").appendTo(table);
      } else if (count == 0) {
        $("<ul>").attr({
          id: "item-shop"
        }).appendTo(table);
        $("<li></li>").addClass("attribute-item").text(clave + ": " + category[clave]).appendTo("#item-shop");
        count = 1;
      } else if (clave != "formats" && clave != "awards" && clave != "clicks" && clave != "likes") {
        $("<li></li>").addClass("attribute-item").text(clave + ": " + category[clave]).appendTo("#item-shop");
      }
    ...
    if (category.likes != null && clave == "likes") {
      $("<svg>").addClass("attribute-item").attr({
    ...
    }).text("comprar").appendTo("#item-shop"); //ACI ACI ACI ACI ACI ACI ACI ACI ACI ACI ACI ACI ACI ACI ACI ACI ACI ACI ACI ACI ACI ACI ACI ACI ACI ACI ACI ACI ACI ACI ACI ACI ACI ACI ACI ACI ACI ACI ACI ACI ACI ACI ACI ACI ACI ACI ACI ACI ACI ACI
    // PRUEBA
...
function loadHomeProducts() {
  ...
  var urlCategory = "module/shop/controller/controllerShopPage.php?op=";
  ...
  switch (op) {
    case "details":
      var homeID = sessionStorage.getItem("id");
    ...
    case "category":
      ...
      switch (filterCategory) {
        case "decade":
          ...
          "listShop";
          pagination("decade");
          searchAjaxProducts(friendlyModFunc("shop", "categoryDecade"), order, offset); // FET AL FRAMEWORK
        ...
        case "formate":
          ...
          pagination("formate");
          searchAjaxProducts(friendlyModFunc("shop", "categoryFormate"), order, offset); // FET AL FRAMEWORK
        ...
        case "genere":
          ...
          pagination("genere");
          searchAjaxProducts(friendlyModFunc("shop", "categoryGenere"), order, offset); // FET AL FRAMEWORK
    ...
    case "search":
      ...
      searchAjaxProducts(friendlyModFunc("search", "searchList"), order, offset, search); //Modul per fer!!!

      break;

    case "filter":
      var filter = sessionStorage.getItem("filter"); // alert(filter);

      filter = window.btoa(unescape(encodeURIComponent(filter)));
      pagination("filterQuery", filter);
      searchAjaxProducts(friendlyModFunc("shop", "filter"), order, offset, filter); // FET AL FRAMEWORK
...
function clickProduct() {
  $("body").on("click", "img", function () {
    ...
    if (clase === "touch") {
      ...
      "module/shop/controller/controllerShopPage.php?op=openProduct&product=", id); // console.log(API_KAY);
      // console.log("la ID es " + id);

      countClickProduct(id); // openMaps(id);
      // APIspam();
      ...
      $("#apispam").empty();
      sessionStorage.setItem("op", sessionStorage.getItem("opStore")); // sessionStorage.removeItem("op");
...
function countClickProduct(id) {
  dirUrl = "module/home/controller/controllerHomePage.php?op=countClick&count=" + id;
  console.log(dirUrl); // return false;

  ajaxPromise(dirUrl, "GET", "JSON").then(function (data) {
    console.log(data);
  })["catch"](function () {
...
function searchAjaxProducts(dirUrl, order, offset) {
  var search = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : undefined;
  var boolTrue = arguments.length > 4 && arguments[4] !== undefined ? arguments[4] : false;
  var idUser = getUser(); // alert(idUser)
  // alert(dirUrl)
  ...
  ajaxPromise(dirUrl, "GET", "JSON", {
    ...
    if (category.length === 1) {
      // FET AL FRAMEWORK
      divsProduct("module/shop/controller/controllerShopPage.php?op=openProduct&product=", category[0]["id"]);
    } else {
      console.log(category);
      $("#listShop").empty();

      if (boolTrue === true) {
        $("<h1></h1>").text("No se han encontrado resultados con esta búsqueda").appendTo("#listShop");
      }
      ...
      for (var i = 0; i < category.length; i++) {
        ...
        var h2 = $("<h2></h2>").text(category[i]["price"] + "€").appendTo(div2); //ACÍ ACÍ ACÍ ACÍ ACÍ ACÍ ACÍ ACÍ ACÍ ACÍ ACÍ ACÍ ACÍ ACÍ ACÍ ACÍ ACÍ ACÍ ACÍ ACÍ ACÍ ACÍ ACÍ ACÍ ACÍ ACÍ ACÍ ACÍ ACÍ ACÍ ACÍ ACÍ ACÍ ACÍ ACÍ ACÍ ACÍ ACÍ ACÍ ACÍ ACÍ ACÍ ACÍ ACÍ ACÍ ACÍ ACÍ ACÍ ACÍ ACÍ ACÍ ACÍ
    ...
    if (boolTrue === true) {
      $("#listShop").empty();
      $("<h1></h1>").text("Ha habido un problema en tu búsqueda").appendTo("#listShop");
    } else {
      searchAjaxProducts(friendlyModFunc("shop", "listShop"), order, 0, undefined, true);
    }
  });
}
...
function genereFilter() {
  ajaxPromise(friendlyModFunc("shop", "filterType"), "GET", "JSON", {
    ...
    $("<option>").attr({
      value: "default"
    }).text("Todos los géneros").appendTo("#genere");

    for (var clave in data) {
      var valueGenere = data[clave]["genere"];
      $("<option>").attr({
        value: valueGenere
      }).text(valueGenere).appendTo("#genere");
    }
  })["catch"](function () {});
}
...
function countryFilter() {
  ajaxPromise(friendlyModFunc("shop", "filterType"), "GET", "JSON", {
    ...
    $("<option>").attr({
      value: "default"
    }).text("Todo el mundo").appendTo("#country");

    for (var clave in data) {
      var valueCountry = data[clave]["country"];
      $("<option>").attr({
        value: valueCountry
      }).text(valueCountry).appendTo("#country");
    }
  })["catch"](function () {});
}
...
function formatsFilter() {
  ...
  for (var i = 0; i < formatsMatrix.length; i++) {
    ...
    $("<label>").append(document.createTextNode(formatsMatrix[i])).appendTo("#filters");
    $("<br>").appendTo("#filters");
  }
}
...
function pagination() {
  var type = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : "listShop";
...
function clickPage() {
  $("#pagination").bootpag({
    ...
    var offset = (num - 1) * 6; // alert(offset);
...
$(document).ready(function () {
  ...
  clickProduct();
  clickPage();
  likeCart("svg"); // like("li"); en principi este ja estava així, si al acabar de migrar-ho tot funciona, a tope!
});