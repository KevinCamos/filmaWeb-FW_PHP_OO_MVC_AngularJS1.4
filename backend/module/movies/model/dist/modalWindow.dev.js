...
function readMovie() {
  $(".movieRead").click(function () {
    var id = this.getAttribute("id"); // console.log(id);//Debugeo para comprobar que entra a la función
    ...
    $.ajax({
      ...
      for (var clave in movie) {
        if (movie.hasOwnProperty(clave) && clave != "img") {
          $("<p></p>").attr({
            "class": "attrModal"
          }).append(document.createTextNode(clave + ":  " + movie[clave])).appendTo(".modalclass");
        } else if (clave == "img") {
          $("<img>").attr({
            "class": "attrModal",
      ...
      $("#list_modal").show();
      $("#list_modal").dialog(_defineProperty({
        width: 600,
        //<!-- ------------- ancho de la ventana -->
        height: 800,
        ...
        hide: 22,
        //<!-- ----------- animación al cerrar la ventana -->
        resizable: "false",
        //<!-- ------ fija o redimensionable si ponemos este valor a "true" -->
        // position: "center",//<!--  ------ posicion de la ventana en la pantalla (left, top, right...) -->
        modal: "true",
        //<!-- ------------ si esta en true bloquea el contenido de la web mientras la ventana esta activa (muy elegante) -->
        buttons: {
          "De Acuerdo": function DeAcuerdo() {
            $(this).dialog("close");
          }
        },
        show: {
  ...
  $(".movieDelete").click(function () {
    ...
    $.ajax({
      ...
      $("<div></div>").attr({
        "class": "modalclass",
        id: "modalclass"
      }).append(document.createTextNode("Seguro que desea eliminar la siguiente película? ")).appendTo("#list_modal");
      ...
      $("#list_modal").dialog(_defineProperty({
        ...
        buttons: {
          ...
          Delete: function Delete() {
            $(this).dialog("close"); // window.location.href = 'module/movies/controller/controller_movies.php?op=delete_modal&modal=' + id;
