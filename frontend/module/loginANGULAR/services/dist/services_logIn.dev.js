...
getyourcar.factory('services_logIn', ['$rootScope', 'services', 'services_localStorage', function ($rootScope, services, services_localStorage) {
  ...
  function printMenu() {
    if (localStorage.token && localStorage.secureSession) {
      services.post('login', 'returnSession', {
        jwt: localStorage.token,
        secureSession: localStorage.secureSession
  ...
  function redirectLogIn(secureSession, JWTToken) {
    var jumpPage = localStorage.jumpPage ? services_localStorage.setJumpPage() : 'home';
  ...
  function logOut() {
    services.get('login', 'logOut').then(function (response) {
      if (response === "Done") {
