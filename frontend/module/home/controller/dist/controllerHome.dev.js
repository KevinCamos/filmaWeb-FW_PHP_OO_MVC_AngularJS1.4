...
function loadDivsCarousel() {
  ...
  $("<h4></h4>").attr({
    "class": "line_menu"
  }).append(document.createTextNode("Categorías")).appendTo("#large_columns");
  $("<div></div>").attr({
...
function loadCategoryCarousel() {
  ajaxPromise(friendlyModFunc("home", "carousel"), "GET", "JSON").then(function (category) {
    console.log(category);

    for (var i = 0; i < category.length; i++) {
      var id = "" + category[i]["id_category"];
      var ObjectCategory = category[i]["category"];
      var img = "" + category[i]["img"];
      var firstDiv = $("<div></div>").attr({
        "class": "item",
        id: "item" + img
...
function ajaxSearch(dirUrl) {
  ...
  ajaxPromise(dirUrl, "GET", "JSON", {
    ...
    for (var i = 0; i < category.length; i++) {
      var id = "" + category[i]["id"]; // let name = category[i]["name"];
...
function dialogBeastProducts() {
  $("<p></p> ").attr({
    id: "font-center-home"
  }).append(document.createTextNode("Nuestros mejores productos")).appendTo("#productsHome");
}
...
function clickCategory() {
  ...
  if (category == null) {
    ...
    console.log("El archivo al que das click no tiene ningún criterio de búsqueda añadido");
  } else {
...
function clickProductHome() {
  ...
  if (id == null) {
    ...
    // setTimetout() es una función para decirle que pasado X tiempo realice una función
    countClickProduct(id); // return false;
...
$(document).ready(function () {
  ...
  loadDivsCarousel(); // loadCategoryCarousel();
