...
function modalDelete(thisProduct) {
  Swal.fire({
    ...
    if (result.isConfirmed) {
      ...
      ajaxUpdateRemoProduct(thisProduct, true, "delete"); // $("#lineProduct" + idProduct).remove();
      // getCart();
    } else {
      return false;
    }
...
function restrictionSumRestDelete(thisProduct, type, idProduct) {
  var deleteQuest2True = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : false;

  if (deleteQuest2True == true) {
    return true;
  }

  cantidad = parseInt($("#totalAmount" + idProduct).text(), 10); // return type;
  // alert(cantidad);
...
function ajaxUpdateRemoProduct(thisProduct) {
  var deleteQuest2True = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : false;
  var typeQuest = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : null;
  console.log(thisProduct);
  var thisClass = $(thisProduct).attr("class");

  if (thisClass == "fas fa-minus" || thisClass == "fas fa-plus" || thisClass == "fas fa-trash-alt") {
    console.log("entra");
    ...
    if (tokenTrue() == true) {
      ...
      var type = id[0];
      var idProduct = id[1];
      var idAlbaran = id[2]; // var idUser = localStorage.getItem("idusers");

      var deleteQuest = restrictionSumRestDelete(thisProduct, type, idProduct, deleteQuest2True);
      ...
      if (deleteQuest == true) {
        ...
        "POST", "JSON", {
          ...
          switch (type) {
            case "rest":
              updateTotalPrice(idProduct, idAlbaran); // sumRestAmount(idProduct, -1);
            ...
            case "delete":
              $("#lineProduct" + idProduct).remove(); // toastr.success("Producto eliminado con éxito!");
...
function updateTotalPrice(idProduct, idAlbaran) {
  ...
  "POST", "JSON", {
    ...
    // alert(data)
    console.log(data); // totalPrice = totalPrice.toFixed(2).replace(".", ",");
    ...
    $("#totalprice" + idProduct).html("<h1 class='linePrice'>" + totalPrice + "€<h2>");
  ...
  $("#buyMe").click(function () {
    ...
    if (tokenTrue() == true) {
      ...
      ajaxPromise(friendlyModFunc("cart", "getAlbaran"), "POST", "JSON", {
        ...
        var idAlbaran = data;
        ajaxPromise(friendlyModFunc("cart", "getTotalCart"), "POST", "JSON", {
          idAlbaran: idAlbaran
        }).then(function (data) {
          // alert("entra:"+data)
          console.log(data);
          continueBuy(data); // totalPrice = totalPrice.toFixed(2).replace(".", ",");
        })["catch"](function (data) {
          alert("NO ENTRA :" + data);
          console.log(data);
        });
...
function continueBuy(data) {
  ...
  for (var i = 0; i < Object.keys(data).length; i++) {
    ...
    $("<td>").text(price + "€").addClass("albaranPrice").appendTo(line);
    $("<td>").text(movie).css("padding-left", "80px").appendTo(line);
    $("<td>").text(totalPriceProduct + "€").appendTo(line);
  }
  ...
  for (var i = 0; i < 3; i++) {
    ...
    switch (i) {
      case BASE_IMPONIBLE:
        ...
        $("<td>").text(BaseImponible + "€").appendTo(line);
        break;
      ...
      case PRECIO_TOTAL:
        ...
        $("<td>").text(totalPrice + "€").appendTo(line);
        break;
    }
  }
...
function returnEndClick(idAlbaran) {
  ...
  $("#endCart").click(function () {
    Swal.fire("¡Compra finalizada!", "Gracias por confiar en nuestos servicios", "success").then(function () {
