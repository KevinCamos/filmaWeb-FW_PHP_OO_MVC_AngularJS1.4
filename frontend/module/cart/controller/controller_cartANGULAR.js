getyourcar.controller('controller_cart', function ($scope, services, toastr, dataCart) {
    ...
    $scope.changeDays = function (carPlate) {
        services.put('cart', 'updateDays', {
                days: $scope.qtyDays[carPlate],
    ...
    $scope.removeDiscCode = function () {
        services.put('cart', 'removeDiscCode', {
                JWT: localStorage.token
            })
            .then(function (response) {
                if (response == 'true') {
    ...
    function updateCart() {
        services.post('cart', 'loadDataCart', {
                JWT: localStorage.token
            })
            .then(function (response) {
