...
function likeCart(etiqueta) {
  $("body").on("click", etiqueta, function () {
    ...
    if (thisClass == "far fa-heart like" || thisClass == "fas fa-heart unlike") {
      like(this);
...
function like(like) {
  ...
  if (tokenTrue() == true) {
    ...
    var typeLike = id[0];
    var idProduct = id[1];
    var idUser = localStorage.getItem("idusers"); // alert(idProduct + " i " + typeLike + "i" + idUser);
...
function cartShop(addCart) {
  ...
  if (tokenTrue() == true) {
    ...
    var idUser = localStorage.getItem("idusers"); // console.log(id + " i " + siteShop + " i " + idUser);
