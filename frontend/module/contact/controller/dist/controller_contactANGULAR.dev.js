...
function loadMap() {
  ...
  var contentString = '<div id="content">' + '<div id="siteNotice">' + '</div>' + '<h4>Ontinyent</h4>' + '<div id="bodyContent">' + '<p><b>Get your Car</b></p>' + '<a href="index.php?page=controllerHomePage&op=list">Home</a>' + '</div>' + '</div>'; //////
...
getyourcar.controller('controller_contact', function ($scope, services, toastr) {
  ...
  $scope.sendEmail = function () {
    var email = {
      ...
      'matter': $scope.email_matter,
      'message': $scope.email_message
    }; //////

    services.post('contact', 'sendEmail', email).then(function (response) {
      if (response == 'true') {
